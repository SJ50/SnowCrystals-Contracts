/**
 *Submitted for verification at cronoscan.com on 2022-05-20
 */

/**
 *Submitted for verification at cronoscan.com on 2022-02-12
 */

/**
 https://cronoscan.com/address/0x3bf476788b951288dad71cadd3716a362ee88030#code
  */
...
contract ScrubBank is ReentrancyGuard {
    ...
    address public timelock =
        address(0x0000000000000000000000000000000000000000); // 6h timelock
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "ScrubBank: caller is not the operator"
        );
        _;
    }

    modifier onlyTimelock() {
        require(timelock == msg.sender, "ScrubBank: caller is not timelock");
        _;
    }

    modifier notInitialized() {
        require(!initialized, "ScrubBank: already initialized");
        _;
    }
    ...
    function setEarlyWithdrawFee(uint256 _pid, uint256 _earlyWithdrawFee)
        public
        onlyOperator
    {
    ...
    function setEarlyWithdrawTime(uint256 _pid, uint256 _earlyWithdrawTime)
        public
        onlyOperator
    {
    ...
    function addRewardPool(address _rewardToken, uint256 _rewardPerSecond)
        public
        nonReentrant
        onlyOperator
    {
        require(
            rewardPoolInfo.length <= 16,
            "ScrubBank: Reward pool length > 16"
        );
        massUpdatePools();
        rewardPoolInfo.push(
            RewardPoolInfo({
    ...
    function updateRewardPerSecond(uint256 _rewardPid, uint256 _rewardPerSecond)
        external
        nonReentrant
        onlyOperator
    {
    ...
    function setUnstakingFrozenTime(uint256 _unstakingFrozenTime)
        external
        nonReentrant
        onlyOperator
    {
        require(
            _unstakingFrozenTime <= 7 days,
            "ScrubBank: !safe - dont lock for too long"
        );
    ...
    function pendingReward(
        ...
        if (block.timestamp > pool.lastRewardTime && sharesTotal != 0) {
            ...
            uint256 _rewardPerSecond = rewardPoolInfo[_rewardPid]
                .rewardPerSecond;
            ...
            _accRewardPerShare = _accRewardPerShare.add(
                _reward.mul(1e18).div(sharesTotal)
            );
        }
        return
            user.shares.mul(_accRewardPerShare).div(1e18).sub(
                user.rewardDebt[_rewardPid]
            );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        for (
            uint256 _rewardPid = 0;
            _rewardPid < _rewardPoolLength;
            ++_rewardPid
        ) {
            uint256 _rewardPerSecond = rewardPoolInfo[_rewardPid]
                .rewardPerSecond;
            uint256 _reward = multiplier
                .mul(_rewardPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            pool.accRewardPerShare[_rewardPid] = pool
                .accRewardPerShare[_rewardPid]
                .add(_reward.mul(1e18).div(sharesTotal));
            pool.lastRewardTime = block.timestamp;
        }
    }
    ...
    function _getReward(uint256 _pid) internal {
        ...
        for (
            ...
            if (!stopRewardPool[_rewardPid]) {
                uint256 _pending = _user
                    .shares
                    .mul(_pool.accRewardPerShare[_rewardPid])
                    .div(1e18)
                    .sub(_user.rewardDebt[_rewardPid]);
                if (_pending > 0) {
                    RewardPoolInfo storage rewardPool = rewardPoolInfo[
                        _rewardPid
                    ];
                    address _rewardToken = rewardPool.rewardToken;
                    safeRewardTransfer(_rewardToken, msg.sender, _pending);
                    rewardPool.totalPaidRewards = rewardPool
                        .totalPaidRewards
                        .add(_pending);
                    emit RewardPaid(
                        _rewardPid,
    ...
    function _checkStrategyBalanceAfterDeposit(
        ...
        require(
            _oldInFarmBalance + _depositAmount <=
                IStrategy(_strategy).inFarmBalance(),
            "Short of strategy infarm balance: need audit!"
        );
        require(
            _oldTotalBalance + _depositAmount <=
                IStrategy(_strategy).totalBalance(),
            "Short of strategy total balance: need audit!"
        );
    }
    ...
    function _checkStrategyBalanceAfterWithdraw(
        ...
        require(
            _oldInFarmBalance <=
                _withdrawAmount + IStrategy(_strategy).inFarmBalance(),
            "Short of strategy infarm balance: need audit!"
        );
        require(
            _oldTotalBalance <=
                _withdrawAmount + IStrategy(_strategy).totalBalance(),
            "Short of strategy total balance: need audit!"
        );
    }
        ...
        notContract
    {
        ...
        if (_wantAmt > 0) {
            ...
            uint256 sharesAdded;
            {
                uint256 _oldInFarmBalance = IStrategy(_strategy)
                    .inFarmBalance();
                uint256 _oldTotalBalance = IStrategy(_strategy).totalBalance();
                sharesAdded = IStrategy(_strategy).deposit(
                    msg.sender,
                    _wantAmt
                );
                _checkStrategyBalanceAfterDeposit(
                    _strategy,
                    _wantAmt,
                    _oldInFarmBalance,
                    _oldTotalBalance
        ...
        for (
            ...
            user.rewardDebt[_rewardPid] = user
                .shares
                .mul(pool.accRewardPerShare[_rewardPid])
                .div(1e18);
        }
        ...
        returns (uint256)
    {
        return
            (whitelisted[_account])
                ? userInfo[_pid][_account].lastStakeTime
                : userInfo[_pid][_account].lastStakeTime + unstakingFrozenTime;
    }

    function earlyWithdrawTimeEnd(uint256 _pid, address _account)
        public
        view
        returns (uint256)
    {
        return
            (whitelisted[_account])
                ? userInfo[_pid][_account].lastStakeTime
                : userInfo[_pid][_account].lastStakeTime +
                    poolInfo[_pid].earlyWithdrawTime;
    }
        ...
        notContract
    {
        ...
        require(user.shares > 0, "ScrubBank: user.shares is 0");
        require(sharesTotal > 0, "ScrubBank: sharesTotal is 0");
        ...
        if (_wantAmt > 0) {
            uint256 sharesRemoved;
            {
                uint256 _oldInFarmBalance = IStrategy(_strategy)
                    .inFarmBalance();
                uint256 _oldTotalBalance = IStrategy(_strategy).totalBalance();
                sharesRemoved = IStrategy(_strategy).withdraw(
                    msg.sender,
                    _wantAmt
                );
                _checkStrategyBalanceAfterWithdraw(
                    _strategy,
                    _wantAmt,
                    _oldInFarmBalance,
                    _oldTotalBalance
            ...
            if (_wantAmt > 0) {
                ...
                if (block.timestamp >= earlyWithdrawTimeEnd(_pid, msg.sender)) {
                    ...
                    uint256 fee = _wantAmt.mul(
                        poolInfo[_pid].earlyWithdrawFee.div(10000)
                    );
    ...
    function setWhitelisted(address _account, bool _whitelisted)
        external
        nonReentrant
        onlyOperator
    {
    ...
    function setStopRewardPool(uint256 _pid, bool _stopRewardPool)
        external
        nonReentrant
        onlyOperator
    {
    ...
    function setPausePool(uint256 _pid, bool _pausePool)
        external
        nonReentrant
        onlyOperator
    {
    ...
    function setTimelock(address _timelock) external {
        require(
            msg.sender == timelock ||
                (timelock == address(0) && msg.sender == operator),
            "ScrubBank: !authorised"
        );
        timelock = _timelock;
    }
    ...
    function executeTransaction(
        ...
        require(
            success,
            "ScrubBank::executeTransaction: Transaction execution reverted."
        );
